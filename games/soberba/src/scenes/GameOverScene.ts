...
export class GameOverScene extends Phaser.Scene {
  ...
  create(data: { score?: number }) {
    ...
    const title = this.add.text(GAME_WIDTH / 2, GAME_HEIGHT / 2 - 70, 'COLLAPSE', {
    ...
    const epitaph = this.add.text(
      GAME_WIDTH / 2,
      GAME_HEIGHT / 2 + 40,
      '"and the tower collapsed under its own weight"',
      {
        fontFamily: 'Georgia, "Times New Roman", serif',
        fontSize: '13px',
        color: '#c9a96b',
    ...
    const hint = this.add.text(GAME_WIDTH / 2, GAME_HEIGHT / 2 + 100, '[ TAP OR PRESS ANY KEY TO RESTART ]', {
      fontFamily: 'Georgia, "Times New Roman", serif',
      fontSize: '12px',
      color: '#a89576',
    })
